...
export const TelemetryTab: React.FC = () => {
  ...
  // Compute totals from service data
  ...
  return (
    <div className="h-full overflow-y-auto flex flex-col gap-6 pb-4 pr-1">
      {/* Three Metric Cards – removed "Active Streams" */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
        
        {/* Card 1: Total Throughput */}
        <div className="bg-white/80 backdrop-blur-md border border-neutral-200/60 rounded-xl p-5 shadow-xs flex flex-col justify-between min-h-[140px] relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between text-neutral-400">
            ...
            <Activity className="w-4 h-4 text-neutral-600 group-hover:text-neutral-900 transition-colors" />
          </div>
          <div className="mt-3 flex items-baseline gap-1.5">
            <motion.div
              ...
              className="text-2xl font-bold text-neutral-900 font-mono tracking-tight"
            >
              {formatEps(totalEps)}
            </motion.div>
            <span className="text-[10px] font-mono text-neutral-400 font-semibold uppercase">evt/s</span>
          </div>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-[11px] text-neutral-500 font-sans">
              Peak: <span className="font-mono text-neutral-700">{peakThroughput.toLocaleString()}</span>
            </span>
            <span className="text-[10px] font-mono bg-[#C7F000]/10 text-neutral-700 px-1.5 py-0.5 rounded-sm font-semibold uppercase">
              ACTIVE FLOW
            </span>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-md border border-neutral-200/60 rounded-xl p-5 shadow-xs flex flex-col justify-between min-h-[140px] relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between text-neutral-400">
            ...
            <Zap className="w-4 h-4 text-neutral-600 group-hover:text-neutral-900 transition-colors" />
          </div>
          <div className="mt-3 flex items-baseline gap-1.5">
            <motion.div
              key={totalDrops}
              initial={{ opacity: 0.8, y: -1 }}
              animate={{ opacity: 1, y: 0 }}
              className={`text-2xl font-bold font-mono tracking-tight ${totalDrops > 0 ? 'text-red-500' : 'text-neutral-900'}`}
            >
              {metrics.connectionStatus === 'connected' ? totalDrops.toLocaleString() : '---'}
            </motion.div>
            <span className="text-[10px] font-mono text-neutral-400 font-semibold uppercase">drops/s</span>
          </div>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-[11px] text-neutral-500 font-sans">
            ...
            <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-sm font-semibold uppercase ${
              totalDrops > 0 ? 'bg-red-100 text-red-700' : 'bg-[#C7F000]/10 text-neutral-700'
            }`}>
        ...
        <div className="bg-white/80 backdrop-blur-md border border-neutral-200/60 rounded-xl p-5 shadow-xs flex flex-col justify-between min-h-[140px] relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between text-neutral-400">
            ...
            <Layers className="w-4 h-4 text-neutral-600 group-hover:text-neutral-900 transition-colors" />
          </div>
          <div className="mt-3 flex items-baseline gap-1.5">
            <motion.div
              key={connectedNodes}
      ...
      {/* Telemetry Summary – uses the same computed values */}
      <div className="bg-white/80 backdrop-blur-md border border-neutral-200/60 rounded-xl p-5 shadow-xs">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-neutral-100">
          ...
          <span className="text-[10px] font-mono bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-xs font-semibold uppercase">
            {metrics.connectionStatus === 'connected' ? 'LIVE' : 'STANDBY'}
          </span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-sans">
          <div>
            <span className="text-neutral-400 block mb-1">Avg Events/sec per Stream</span>
            <span className="font-mono font-bold text-neutral-800">
              {metrics.connectionStatus === 'connected' ? Math.round(totalEps / (activeStreams || 1)).toLocaleString() : '---'}
            </span>
          </div>
          <div>
            <span className="text-neutral-400 block mb-1">Total Subscribers</span>
            <span className="font-mono font-bold text-neutral-800">
              {metrics.connectionStatus === 'connected' ? streams.reduce((sum, s) => sum + s.subscribers, 0) : '---'}
            </span>
          </div>
          <div>
            <span className="text-neutral-400 block mb-1">Avg Buffer Utilization</span>
            <span className="font-mono font-bold text-neutral-800">
              {metrics.connectionStatus === 'connected' ? `${Math.round(streams.reduce((sum, s) => sum + s.bufferUtilizationPercent, 0) / (activeStreams || 1))}%` : '---'}
            </span>
          </div>
          <div>
            <span className="text-neutral-400 block mb-1">Connected Sensors</span>
            <span className="font-mono font-bold text-neutral-800">
